...
const ClubStudioViewer = ({ route, navigation }) => {
  const { itemData, clubId } = route.params;
  ...
  useEffect(() => {
    ...
    const subscriber = Fire.getStudioContent(clubId, itemData, (content) => {
  ...
  const ratingCompleted = async (rating) => {
    ...
    await Fire.updateStudioContent(clubId, itemData, { rating: ratings, reviews: review });
  };

  //Add Comment
  const addComment = async () => {
    setloading((loading) => true);
    await Fire.addComment(clubId, itemData, comment);
        ...
        <LinearGradient colors={["#4e4376", "#4e4376"]}>
          ...
          {content.userId != Fire.uid ? (
            <View style={styles.section}>
              <Text style={styles.sectionTitle}> قيم المحتوى </Text>
              <AirbnbRating
                count={5}
...
export default ClubStudioViewer;
