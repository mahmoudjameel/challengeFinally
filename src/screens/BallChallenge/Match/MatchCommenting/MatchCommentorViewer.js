...
const MatchCommentorViewer = ({ route, navigation }) => {
  ...
  useEffect(() => {
    let isMounted = true;
    //get Content information
    const subscriber = Fire.getMatchCommentingContent(matchId, itemData, (content) => {
  ...
  const ratingCompleted = async (rating) => {
    ...
    await Fire.updateMatchCommentingContent(matchId, itemData, { rating: ratings, reviews: review });
  };

  //Add Comment
  const addComment = async () => {
    setloading((loading) => true);
    await Fire.addCommentToMatchCommenting(matchId, itemData, comment);
...
export default MatchCommentorViewer;
