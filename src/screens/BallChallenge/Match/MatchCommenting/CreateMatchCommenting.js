...
const CreateMatchCommenting = ({ route, navigation }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      setBalls(parseInt(b.recordMatchCommenting || 1));
      setUser(user);
    })();
  }, []);
  ...
  const uploadContent = async () => {
    ...
    Alert.alert(
      I18n.t('alert'),
      `${I18n.t('Topostwithdrawn')} ${balls} ${I18n.t('goldens')}`,
      [
        { text: I18n.t('cancel'), style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            if (balls <= user.goldBalls) {
              const response = await Fire.addMatchCommentingContent(matchId, {
                title: title,
                type: "video/mp4",
                imageVideo: video,
                rating: 0,
              ...
              const res = await Fire.updateUser(user.uid, {
                goldBalls: firebase.firestore.FieldValue.increment(-balls),
              });
  ...
  const renderInner = () => (
    <ScrollView contentContainerStyle={styles.panel}>
      <View style={{ alignItems: "center" }}>
        <Text style={styles.panelTitle}>{I18n.t('uploadvideo')}</Text>
      </View>
      <TouchableOpacity style={styles.panelButton} onPress={takePhotoFromCamera}>
        <Text style={styles.panelButtonTitle}>{I18n.t('videorecording')}</Text>
      </TouchableOpacity>
              ...
              <View style={{ alignItems: "center" }}>
                ...
                >
                  <View style={styles.imageContainer}>
                    <ImageBackground source={{ uri: video }} style={{ height: 200, width: 200 }}>
...
export default CreateMatchCommenting;
