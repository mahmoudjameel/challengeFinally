...
const MatchAnalysisViewer = ({ route, navigation }) => {
  ...
  useEffect(() => {
    let isMounted = true;
    //get Content information
    const subscriber = Fire.getMatchAnalysisContent(matchId, itemData, (content) => {
      if (isMounted && content != null) {
        setContent(content);
        if (challengeUser == null) Fire.getUser(content.userId).then((user) => setChallengeUser(user));
      }
    });
  ...
  const ratingCompleted = async (rating) => {
    ...
    await Fire.updateMatchAnalysisContent(matchId, itemData, { rating: ratings, reviews: review });
  };

  //Add Comment
  const addComment = async () => {
    setloading((loading) => true);
    await Fire.addCommentToMatchAnalysis(matchId, itemData, comment);
        ...
        <LinearGradient colors={["#4e4376", "#4e4376"]}>
          ...
          >
            {content.comments != null ? (
              ...
              <View style={{ padding: 10 }}>
                <Text style={styles.sectionContent}>{I18n.t('CreateComment')}</Text>
              </View>
            )}
          </ScrollView>
          ...
          {content.userId != Fire.uid ? (
            <SendGiftDialog
              ...
              visible={dialogState}
              onPressOutside={() => setDialogState(false)}
            />
          ) : null}
        </LinearGradient>
      </ImageHeaderScrollView>
...
export default MatchAnalysisViewer;
