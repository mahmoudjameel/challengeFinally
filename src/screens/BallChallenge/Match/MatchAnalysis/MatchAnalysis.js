...
const MatchAnalysis = ({ route, navigation }) => {
  ...
  useEffect(() => {
    let isMounted = true;
    const subscriber = Fire.getAllMatchAnalysisContent(matchId, (content) => {
      if (isMounted) {
        setState((prevState) => {
    ...
    return (
      <View style={styles.button}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate(I18n.t('Creatmathematicalanalysis'))}
        >
          <LinearGradient
      ...
      <Animatable.View animation="bounceIn">
        <FlatList
          ...
          renderItem={({ item }) => (
            <StudioContentCard
              item={item}
              onPress={() => navigation.navigate(I18n.t('Analysispage'), { itemData: item.uid })}
            />
          )}
        />
      </Animatable.View>
