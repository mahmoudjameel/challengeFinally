...
const MatchRefereeMistake = ({ route, navigation }) => {
  ...
  useEffect(() => {
    let isMounted = true;
    const subscriber = Fire.getAllRefereeMistakesContent(matchId, (content) => {
      if (isMounted) {
        setState((prevState) => {
    ...
    return (
      <View style={styles.button}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate(I18n.t('Publicationarbitralerror'))}
        >
          <LinearGradient
      ...
      <Animatable.View animation="bounceIn">
        <FlatList
          ...
          renderItem={({ item }) => (
            <StudioContentCard
              item={item}
              onPress={() => navigation.navigate(I18n.t('arbitrationerrorpage'), { itemData: item.uid })}
            />
          )}
        />
      </Animatable.View>
...
export default MatchRefereeMistake;
