...
const Login = ({ navigation }) => {
  ...
  const [data, setData, user] = React.useState({
    username: "",
    password: "",
    name: "",
    image: "",
      ...
      if (result.type === 'success') {
        ...
        .then(result => {
          ...
          if (result.additionalUserInfo.isNewUser) {
            // builds user info at users node if user is new user
            firebase
      .database()
      .collection('Users')
      .add({
        ...
        image: user?.photoURL      })
            ...
            //updates user data in user node to show last_logged_in time
            firebase
              .database()
    ...
    if (data.loading) {
      ...
      return (
        <View>
          ...
          {!googleSubmitting && (
          <TouchableOpacity style={styles.button} onPress={Glogin}>
            <LinearGradient
              colors={["#02aab0", "#00cdac"]}
              start={{ x: 0, y: 0 }}
              end={{ x: 1, y: 1 }}
              style={styles.signInG}
            >
            <Fontisto name="google" size={25} color={primary} style={{marginRight:5}}/>
      ...
      <Animatable.View style={styles.footer} animation="fadeInUpBig">
        <View style={styles.radio}>
          ...
          <View style={styles.text}>
            ...
            />
            <Text style={{ marginTop: 7, color: "#fff" }}>{I18n.t('Phone')}</Text>
          </View>
        </View>
